...
rifX = sizeX / 2
rifY = sizeY / 2
rifZ = 0.4  # Adjusted for better relative depth measurement
...
try:
    while True:
        ...
        if results.pose_landmarks:
            ...
            error_X = -(rifX - nose[0])
            error_Y = rifY - nose[1]
            error_Z = rifZ - nose[2]  # Adjusted to correctly calculate distance error
            ...
            if pose_action == "TURN_LEFT":
                ...
                if abs(uZ) > 0.1:  # Forward/Backward threshold adjusted for better sensitivity
                    if uZ > 0:
                        set_dc_motor(dc_motor_speed, "FORWARD")
